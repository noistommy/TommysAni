...
(function(window, document, undefined) {
	/*
	[min]
	use strict는  ECMAScript 5 버전에 있는 새로운 기능으로써, 
	당신의 프로그램 또는 함수를 엄격한 운용 콘텍스트 안에서 실행시킬 수 있게끔 합니다.
	이 엄격한 콘텍스트는 몇가지 액션들을 실행할 수 없도록 하며, 좀 더 많은 예외를 발생시킵니다.
	예외-(코딩 실수/전역객체 접근/중복 코드 등)
	http://blog.aliencube.org/ko/2014/01/02/reasons-behind-using-strict-mode-while-coding-javascript/ >> 참고
	*/
	'use strict';
	...
	//Minify optimization.[min]최적화를 위한 변수 치환
	//[min]hasOwnProperty : 객체에 메소드가 존재하는지 확인하는 방법 
	//객체 자신의 메소드 인지 확인 또다른 방법으로 'in'이 있다. in연산자는 상속계통을 다 뒤져서 존재하는지 확인한다.
	...
	//Will be called once (when skrollr gets initialized).[min]CSSPrefix 찾는 함수
	...
	//[min]requestAnimationFrame : 에니메이션용 타이밍 컨트롤
	//시스템이 프레임을 그릴 준비가 되었을때 애니메이션 프레임을 호출하여 웹 페이지를 만들수 있는 효울적인 방법을 제공한다.
	//이전 사용하던 settimeout,setinterval 에서 발생하던 문제를 해결해준다.(프레임 손실,cup 과도 사용 등)
	//에니메이션을 지속시키기 위해 콜백함수 내에서 재호출한다.
	...
	//Built-in easing functions.[min]내장 easing 함수
	var easings = {
		begin: function() {
			return 0;
		},
	 ...
	 * Transform "relative" mode to "absolute" mode.
	 [min]relative를 absolute로 바꿔주는 함수.
	 getBoundingClientReck()은 Element의 offset값(top/bottom/left/right/width/height)을 반환한다.
	 ...
	 * Animates scroll top to new position.
	 [min]새로운 위치에서 부터 에니메이션 시키기 위한 함수.
	 */
 ...
 (function(document, window) {
	...
	*/
 	var findParentLink = function(element) {
 		...
 		if (element.tagName === 'A' || element.tagName === 'text' || element.tagName === 'circle') {
 			return element;
 		}
